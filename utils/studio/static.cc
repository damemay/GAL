...
int main(int argc, char* argv[]) {
    ...
    while(sdl.is_running()) {
            ...
            for(size_t i=0; i<scene.get_objects().size(); i++) {
                if(ImGui::TreeNode(scene.get_objects().at(i)->get_model()->get_name().data())) {
                    ...
                    if(ImGui::InputFloat("X", &t->x, 0.1f) || ImGui::InputFloat("Y", &t->y, 0.1f) || ImGui::InputFloat("Z", &t->z, 0.1f))
                        obj->translate(glm::vec3(t->x,t->y,t->z));
                    if(ImGui::InputFloat("RAD", &t->rad, 0.1f) || ImGui::InputFloat("X", &t->rx, 0.1f) || ImGui::InputFloat("Y", &t->ry, 0.1f) || ImGui::InputFloat("Z", &t->rz, 0.1f))
                        obj->rotate(t->rad, glm::vec3(t->rx, t->ry, t->rz));
                    ImGui::TreePop();
                }
            }
            ImGui::TreePop();
        ...
        if(ImGui::TreeNode("Import static")) {
            static char buf[2048];
            ImGui::InputText("path", buf, IM_ARRAYSIZE(buf));
            if(ImGui::Button("Load static")) {
                scene.new_object(new glp::Object::Renderable{buf, shader, shading_t});
                std::string sbuf = buf;
        ...
        if(ImGui::TreeNode("Import animated")) {
            static char bufa0[2048];
            ImGui::InputText("path", bufa0, IM_ARRAYSIZE(bufa0));
            static char bufa1[2048];
            ImGui::InputText("path", bufa1, IM_ARRAYSIZE(bufa1));
            if(ImGui::Button("Load")) {
                scene.new_object(new glp::Object::Animated{bufa0, bufa1, sdl.get_dt_ptr(), shader, shading_t});
                std::string sbuf = bufa0;
                scene.get_objects().back()->get_model()->set_name(sbuf.substr(sbuf.find_last_of('/'), sbuf.length())+std::to_string(scene.get_objects().size()));
                transforms.push_back(new transform);
            }
            ImGui::TreePop();
